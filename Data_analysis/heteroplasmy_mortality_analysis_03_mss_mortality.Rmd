...
coef_all <- summary(all)$coef[1, 1:3]
coef_all
```
...
coef_infection <- summary(infection)$coef[1, 1:3]
coef_infection
```
...
coef_neoplasm <- summary(neoplasm)$coef[1, 1:3]
coef_neoplasm
```
...
coef_blood <- summary(blood)$coef[1, 1:3]
coef_blood
```
...
coef_endocrine <- summary(endocrine)$coef[1, 1:3]
coef_endocrine
```
...
coef_mental <- summary(mental)$coef[1, 1:3]
coef_mental
```
...
coef_neuro <- summary(neuro)$coef[1, 1:3]
coef_neuro
```
...
coef_cardio <- summary(cardio)$coef[1, 1:3]
coef_cardio
```
...
coef_pulmo <- summary(pulmo)$coef[1, 1:3]
coef_pulmo
```
...
coef_covid <- summary(covid)$coef[1, 1:3]
coef_covid
```
...
coef_external <- summary(external)$coef[1, 1:3]
coef_external
```
...
data <- bind_cols(
  ...
  dplyr::select(
    `Cause of death`, Events, ` `, `HR (95% CI)`, everything()
  ) %>%
  filter(Events >= 100) %>%
  arrange(-coef) %>%
  mutate(
...
### Figure 6a  
```{r analysis_03 figure6a, fig.height=5, fig.width=9}
...
coef_cancer <- summary(cancer)$coef[1, 1:3]
coef_cancer
```
...
coef_cancer_head_neck <- summary(cancer_head_neck)$coef[1, 1:3]
...
coef_cancer_gi <- summary(cancer_gi)$coef[1, 1:3]
coef_cancer_gi
```
...
coef_cancer_resp <- summary(cancer_resp)$coef[1, 1:3]
coef_cancer_resp
```
...
coef_cancer_bone <- summary(cancer_bone)$coef[1, 1:3]
coef_cancer_bone
```
...
coef_cancer_skin <- summary(cancer_skin)$coef[1, 1:3]
coef_cancer_skin
```
...
coef_cancer_soft_tissue <- summary(cancer_soft_tissue)$coef[1, 1:3]
...
coef_cancer_breast <- summary(cancer_breast)$coef[1, 1:3]
coef_cancer_breast
```
...
coef_cancer_female <- summary(cancer_female)$coef[1, 1:3]
coef_cancer_female
```
...
coef_cancer_male <- summary(cancer_male)$coef[1, 1:3]
coef_cancer_male
```
...
coef_cancer_urinary <- summary(cancer_urinary)$coef[1, 1:3]
coef_cancer_urinary
```
...
coef_cancer_cns <- summary(cancer_cns)$coef[1, 1:3]
coef_cancer_cns
```
...
coef_cancer_endo <- summary(cancer_endo)$coef[1, 1:3]
coef_cancer_endo
```
...
coef_cancer_uns <- summary(cancer_uns)$coef[1, 1:3]
coef_cancer_uns
```
...
coef_cancer_blood <- summary(cancer_blood)$coef[1, 1:3]
coef_cancer_blood
```
...
coef_cancer_lymphoma <- summary(cancer_lymphoma)$coef[1, 1:3]
...
coef_cancer_leukemia <- summary(cancer_leukemia)$coef[1, 1:3]
...
cod_cancer_coef <- rbind(
  coef_cancer, coef_cancer_blood, coef_cancer_head_neck, coef_cancer_gi, coef_cancer_resp, coef_cancer_bone, coef_cancer_skin, coef_cancer_soft_tissue, coef_cancer_breast, 
  coef_cancer_female, coef_cancer_male, coef_cancer_urinary, coef_cancer_cns, coef_cancer_endo, coef_cancer_uns, coef_cancer_lymphoma, coef_cancer_leukemia
  ) %>%
  as_tibble() 
cod_cancer_coef
```
...
data_cancer <- bind_cols(
  cod_cancer_table, cod_cancer_coef
  ) %>%
  mutate(
    lci = exp(coef - 1.96*`se(coef)`),
    uci = exp(coef + 1.96*`se(coef)`),
...
### Figure 6b  
```{r analysis_03 figure6b, fig.height=5, fig.width=9}
