...
cancer_long %<>%
  mutate(
    cancer_dx_male_prostate = 
      case_when(
        grepl("C61", cancer_icd10) ~ 1,
        grepl("185", cancer_icd9) ~ 1,
        TRUE ~ 0
      )
  )

table(cancer_long$cancer_dx_male_prostate, useNA = "ifany")

# date of diagnosis  
cancer_long %<>%
  mutate(
    cancer_date_male_prostate = as.Date(ifelse(cancer_dx_male_prostate == 1, cancer_date, NA), origin = "1970-01-01")
  ) %>%
  arrange(eid, cancer_date_male_prostate) %>%
  # the first date of diagnosis variable (`cancer_fdate_male_prostate`)
  group_by(eid) %>%
  mutate(nrow_male_prostate = row_number(),
         cancer_fdate_male_prostate = as.Date(ifelse(nrow_male_prostate == 1, cancer_date_male_prostate, NA), origin = "1970-01-01")) %>%
  ungroup()
...
cancer_long %<>%
  group_by(eid) %>%
  mutate(
    cancer_yn_male_prostate =
      max(cancer_dx_male_prostate)
  ) %>%
  ungroup() 

table(cancer_long$cancer_dx_male_prostate, useNA = "ifany")
table(cancer_long$cancer_yn_male_prostate, useNA = "ifany")

# save the object for prostate cancer
cancer_male_prostate <- cancer_long %>%
  filter(nrow_male_prostate == 1) %>%
  dplyr::select(
    eid, cancer_dx_male_prostate, cancer_date_male_prostate, cancer_age 
    ) %>%
  rename(cancer_age_male_prostate = cancer_age)

# remove variables related to prostate cancer from `cancer_long` data set  
cancer_long %<>%
  dplyr::select(-c(cancer_dx_male_prostate, cancer_date_male_prostate, nrow_male_prostate, cancer_fdate_male_prostate, cancer_yn_male_prostate))
```
...
cancer_wide <- cancer_neoplasm %>%
  ...
  left_join(., cancer_male_prostate, by = "eid") %>%  
