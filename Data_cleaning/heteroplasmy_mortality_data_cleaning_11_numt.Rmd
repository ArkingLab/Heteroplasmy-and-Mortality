---
title: "Heteroplasmy - health outcomes - data cleaning"
subtitle: "008 - mito score and mt haplotypes excluding possible NUMTs"
author: "Eliseo Guallar and Yun Soo Hong"
...
```{r clean_11 set up, include = FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Load libraries
```{r clean_11 load libraries, echo = TRUE, message = FALSE}
library(tidyverse)
library(magrittr)
library(lubridate)
library(stringr)
...
```{r clean_11 load data mt_haplotype}
...
```{r clean_11 load data mt_haplogroup}
...
```{r clean_11 load exclusion list}
# suspect 
suspect <- read_table(
...
# suspect2
suspect2 <- read_table(
  "../../posted/data/sample.exclusions/mutect2.05.suspicious.tab1",
  col_names = FALSE
  ) %>%
  mutate(
    eid = str_split(
...
# exclude 
exclude <- read_table(
  "../../posted/data/sample.exclusions/exclude.list_May2022"
  ) %>%
  rename(
    eid = subject
  ) %>%
...
# samples  
samples <- read_table(
  "../../posted/data/sample.exclusions/200k_samples.txt"
  ) %>%
  rename(
    eid = sample
  ) %>%
...
## Identify bad samples to be removed  
```{r clean_11 exclude list}
...
```{r clean_11 apply exclusion}
table(raw$filter_numt, useNA = "ifany")
...
het_cn %>%  # n = 208332 observations (197410 participants)  
  summarise(
    nrow = n(),
    n = n_distinct(eid)
  )
...
## Exclude any potential NUMTs  
* `filter_numt != "numt" | is.na(filter_numt)`  
```{r clean_11 exc filter_numt}
het_cn_all %<>%
  filter(filter_numt != "numt" | is.na(filter_numt))

het_cn_all %>%  # n = 197108 observations (186551 participants)
  summarise(
    nrow = n(),
    n = n_distinct(eid)
  )

table(het_cn_all$filter_numt, useNA = "ifany")
```

# Selection from the data set  
```{r clean_11 select}
set.seed(20220913)
...
## Mitoscore and mitoscore sum (MSS)  
```{r clean_11 mitoscore}
summary(mito$mito_lc_score)
mito %>%
  ggplot() +
  geom_histogram(aes(x = mito_lc_score))

# Mitoscore sum (MSS)
summary(mito$mito_score_sum_lc)
mito %>%
  ggplot() +
  geom_histogram(aes(x = mito_score_sum_lc))
```

## Heteroplasmy count  
```{r clean_11 het_count}
table(mito$het_count, useNA = "ifany")
mito %>%
  ggplot() +
  geom_bar(aes(x = het_count))
```

## Mitochondrial haplotypes  
```{r clean_11 mt_haplotype}
...
* There are discrepancies but to be consistent with Stephanie, I will use the haplogroups generated from `mutect2.haplogroup1_200k.tab`.  

```{r clean_11 mt_haplogroup}
mito %<>%
  mutate(
    hap = fct_collapse(
      haplogroup,
...
mito %>%
  saveRDS("ukb_heteroplasmy_mortality_11_numt.rds")
```
